...
def dedup(ctx: typer.Context) -> None:
    """Merge duplicate app/endpoint records (fix NULL vs NULL duplicates)."""
    import sqlite3
    settings: Settings = ctx.obj
    ...
    merged_ep = 0
    dups = conn.execute("""
        SELECT e.ip, COUNT(*), MIN(e.id) FROM endpoint e
        GROUP BY e.ip HAVING COUNT(*) > 1
    """).fetchall()
    for ip, count, keep_id in dups:
        dup_ids = [r[0] for r in conn.execute(
            "SELECT id FROM endpoint WHERE ip = ? AND id != ?", (ip, keep_id)
        ).fetchall()]
        ph = ",".join("?" for _ in dup_ids)
        conn.execute(f"UPDATE flow SET endpoint_id = ? WHERE endpoint_id IN ({ph})", (keep_id, *dup_ids))
        conn.execute(f"DELETE FROM endpoint WHERE id IN ({ph})", dup_ids)
        merged_ep += len(dup_ids)

    merged_app = 0
    dups = conn.execute("""
        SELECT a.name, COUNT(*), MIN(a.id) FROM app a
        GROUP BY a.name HAVING COUNT(*) > 1
    """).fetchall()
    for name, count, keep_id in dups:
        dup_ids = [r[0] for r in conn.execute(
            "SELECT id FROM app WHERE name = ? AND id != ?", (name, keep_id)
        ).fetchall()]
        ph = ",".join("?" for _ in dup_ids)
        conn.execute(f"UPDATE flow SET app_id = ? WHERE app_id IN ({ph})", (keep_id, *dup_ids))
        conn.execute(f"UPDATE agg_hourly SET app_id = ? WHERE app_id IN ({ph})", (keep_id, *dup_ids))
        conn.execute(f"DELETE FROM app WHERE id IN ({ph})", dup_ids)
        merged_app += len(dup_ids)

    conn.commit()
    # Replace existing (non-unique) indexes with UNIQUE to prevent recurrences
    conn.execute("DROP INDEX IF EXISTS idx_app_name")
    conn.execute("DROP INDEX IF EXISTS idx_endpoint_ip")
    ...
    conn.commit()
    print(f"Merged {merged_ep} duplicate endpoints, {merged_app} duplicate apps")
    print("UNIQUE indexes created — duplicates will not recur.")
