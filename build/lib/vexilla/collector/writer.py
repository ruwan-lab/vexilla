...
class FlowWriter:
    ...
    def poll(self) -> None:
        ...
        logger.debug("Poll: %d external connections from /proc/net", len(conns))

        # Phase 2: Read conntrack for byte counters
        conntrack_entries = read_conntrack()
        ct_map = self._build_ct_map(conntrack_entries)
        logger.debug("Poll: %d conntrack entries", len(conntrack_entries))
        ...
        for conn in conns:
            ...
            proc_info = self._process_resolver.resolve(conn.inode)
    ...
    def _get_or_create_app(self, proc: ProcInfo) -> int:
        ...
        cursor = self._db.execute(
            """INSERT INTO app (name, exe_path, first_seen, last_seen)
               VALUES (?, ?, ?, ?)
               ON CONFLICT(name, exe_path) DO UPDATE SET last_seen = ?
               RETURNING id""",
            (proc.comm, proc.exe_path, self._now, self._now, self._now),
        )
        row = cursor.fetchone()
        if row is not None:
            return row[0]

        # Fallback for SQLite < 3.35 (RETURNING not supported)
        cursor = self._db.execute(
            "SELECT id FROM app WHERE name = ? AND exe_path IS ?",
        ...
        # Insert again (racily safe with UNIQUE constraint)
        cursor = self._db.execute(
            "INSERT OR IGNORE INTO app (name, exe_path, first_seen, last_seen) VALUES (?, ?, ?, ?)",
            (proc.comm, proc.exe_path, self._now, self._now),
        )
        self._db.execute(
            "UPDATE app SET last_seen = ? WHERE name = ? AND exe_path IS ?",
            (self._now, proc.comm, proc.exe_path),
        )
    ...
    def _get_or_create_endpoint(self, conn: ProcConn) -> int:
        ...
        cursor = self._db.execute(
            """INSERT INTO endpoint (ip, domain, name_source, first_seen, last_seen)
               VALUES (?, ?, ?, ?, ?)
               ON CONFLICT(ip, domain) DO UPDATE SET last_seen = ?
               RETURNING id""",
            (conn.remote_ip, domain, name_source, self._now, self._now, self._now),
        )
        row = cursor.fetchone()
        if row is not None:
            return row[0]
        ...
        # Fallback
        cursor = self._db.execute(
            "SELECT id FROM endpoint WHERE ip = ? AND domain IS ?",
            (conn.remote_ip, domain),
        )
        row = cursor.fetchone()
        if row is not None:
            return row[0]
        ...
        self._db.execute(
            "INSERT OR IGNORE INTO endpoint (ip, domain, name_source, first_seen, last_seen) VALUES (?, ?, ?, ?, ?)",
            (conn.remote_ip, domain, name_source, self._now, self._now),
        )
        self._db.execute(
            "UPDATE endpoint SET last_seen = ? WHERE ip = ? AND domain IS ?",
            (self._now, conn.remote_ip, domain),
        )
        row = self._db.execute(
            "SELECT id FROM endpoint WHERE ip = ? AND domain IS ?",
            (conn.remote_ip, domain),
